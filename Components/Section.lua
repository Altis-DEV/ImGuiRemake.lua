...
function Section.new(parent, options)
    ...
    self.TitleFrame.Parent = self.Container

    self.ToggleButton = Instance.new("TextButton")
    self.ToggleButton.Name = "ToggleButton"
    self.ToggleButton.Size = UDim2.new(0, HEADER_HEIGHT, 1, 0)
    ...
    self.ToggleButton.Rotation =
        self.Opened and OPEN_ROTATION or CLOSED_ROTATION
    ...
    self.ToggleButton.Parent = self.TitleFrame

    self.TitleLabel = Instance.new("TextLabel")
    self.TitleLabel.Name = "Title"
    self.TitleLabel.Size =
        UDim2.new(1, -HEADER_HEIGHT - 8, 1, 0)
    self.TitleLabel.Position =
        UDim2.new(0, HEADER_HEIGHT + 8, 0, 0)
    ...
    self.ContentFrame = self.ElementContainer

    self.ElementLayout = Instance.new("UIListLayout")
    self.ElementLayout.Name = "ElementLayout"
    self.ElementLayout.FillDirection = Enum.FillDirection.Vertical
    self.ElementLayout.SortOrder = Enum.SortOrder.LayoutOrder
    self.ElementLayout.Padding = UDim.new(0, ELEMENT_GAP)
    self.ElementLayout.Parent = self.ElementContainer

    self.ContentPadding = Instance.new("UIPadding")
    self.ContentPadding.Name = "ContentPadding"
    self.ContentPadding.PaddingLeft = UDim.new(0, INDENT)
    self.ContentPadding.PaddingRight = UDim.new(0, PADDING_RIGHT)
    self.ContentPadding.PaddingTop = UDim.new(0, PADDING_TOP)
    self.ContentPadding.PaddingBottom = UDim.new(0, PADDING_BOTTOM)
    self.ContentPadding.Parent = self.ElementContainer
...
function Section:_RefreshHeight(animated)
    ...
    local targetHeight = self:_GetContentHeight()
    ...
    if animated then
        ...
        self.ElementContainer.Size = targetSize
    end
...
function Section:SetFont(fontType)
    ...
    if typeof(fontType) == "string"
        ...
        if ok and customFont then
            self.ToggleButton.FontFace = customFont
            self.TitleLabel.FontFace = customFont
        end

    elseif typeof(fontType) == "EnumItem"
        and fontType.EnumType == Enum.Font then

        self.ToggleButton.Font = fontType
    ...
    for _, element in ipairs(self.Elements) do
        if element and element.SetFont then
            ...
            if not ok then
                warn(
                    "Section child font update failed:",
                    err
                )
            end
        end
...
function Section:UpdateTheme(theme)
    ...
    for _, element in ipairs(self.Elements) do
        if element and element.UpdateTheme then
            ...
            if not ok then
                warn(
                    "Section child theme update failed:",
                    err
                )
            end
        end
